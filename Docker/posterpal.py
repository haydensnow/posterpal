...
# Set up logging
log_filename = '/var/log/posterpal/log.txt'
...
def load_config_from_env():
    return {
        ...
        'pmm_assets': os.getenv('PMM_ASSETS', 'False').lower() in ('true', '1', 't'),
        'create_backup': os.getenv('CREATE_BACKUP', 'False').lower() in ('true', '1', 't')
    }
...
def process_files(config):
    ...
    if not assets_bool:
        ...
        clear_processing_directory(process)
        rename_episode_images(assets_dir)
...
if __name__ == "__main__":
    try:
        ...
        logging.info("PosterPal script completed successfully.")
    except Exception as e:
